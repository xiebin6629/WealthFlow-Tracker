    ...
    switch (colKey) {
      case 'symbol':
        return isEditing ? (
          ...
          <>
            <div className="font-bold text-slate-900 text-lg">{asset.symbol}</div>
            <div className="text-sm text-slate-500 mb-1">{asset.name}</div>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded ${asset.currency === 'USD' ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700'}`}>
              {asset.currency}
            </span>
          </>
        );
      ...
      case 'category':
        return isEditing ? (
          ...
          <span className="text-sm px-2 py-1 rounded bg-slate-100 text-slate-600">
            {asset.category}
          </span>
        );
      ...
      case 'quantity':
        return isEditing ? (
          ...
          <div>
            {isPen ? (
              <div className="text-xs text-slate-500">{asset.pensionConfig?.startDate}</div>
      ...
      case 'currentValueUsd':
        return <div className="font-medium text-slate-500">{fmt(asset.currentValueUsd, 'USD')}</div>;

      case 'currentValueMyr':
        return <div className="font-bold text-slate-900">{fmt(asset.currentValueMyr, 'MYR')}</div>;

      case 'currentAllocationPercent':
        return <div className="text-sm font-semibold bg-slate-100 py-1.5 px-3 rounded-full inline-block">{asset.currentAllocationPercent.toFixed(1)}%</div>;
    ...
    >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left" style={{ color: 'var(--text-secondary)' }}>
          ...
          {/* Dynamic Footer */}
          {assets.length > 0 && (
            <tfoot className="bg-slate-100 border-t-2 border-slate-200 font-bold text-slate-800 text-sm">
              <tr>
      ...
      {topUpState && (
        <div className="absolute inset-0 z-20 bg-slate-900/10 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-slate-200 w-full max-w-sm overflow-hidden animate-slide-in-right">
